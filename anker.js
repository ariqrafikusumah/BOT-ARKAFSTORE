...
const vcard = 'BEGIN:VCARD\n'
            + 'VERSION:3.0\n'
            + 'FN:Ariq Rafi Kusumah\n'
            + 'ORG:Owner;\n'
            + 'TEL;type=CELL;type=VOICE;waid=6281281444295:+62 812-8144-4295\n'
            + 'END:VCARD'
...
rmenu = "HALLO KAK😊"
botinfo = "MAU SEWA DONASI DULU YA KAK:)"
limitt = 9999999999
memberLimit = 2
ban = ["628385245511@s.whatsapp.net"]
premium = ["6281281444295@s.whatsapp.net"]
...
async function starts() {
	...
	dms.on('chat-update', async (mek) => {
		try {
			...
			const BotWea2 = ["081281444295@s.whatsapp.net"] // ubah aja gapapa
			...
			const ownerNumber = ["6281281444295@s.whatsapp.net"] // owner number ubah aja
		...
		case prefix+'daftar':
			...
			//if (!isPublic) return reply(mess.only.publikG)
				...
				case prefix+'info':
					...
					user.push(sender)
					uptime = process.uptime()
					teks = `➽ *Nama Bot* : ${me.name}\n➽ *Owner Bot* : wa.me/6281281444295\n➽ *prefix* : | ${prefix} |\n➽ *Total Block* : ${blocked.length}\n➽ *Aktif Sejak* : ${kyun(uptime)}\n\n➽ *Total Pengguna* : ${user.length} User\n➽ *Instagram* : https://www.instagram.com/ariqrafi13\n➽ *Special Thanks To* :\n\n➽ Allah SWT \n➽ Ariq Rafi Kusumah\n➽ ARKAFSTORE\n➽https://github.com/ariqrafikusumah`
         ...
         case prefix+'lirik':
                ...
                //if (!isPublic) return reply(mess.only.publikG)
                    ...
                    dms.sendMessage('6281281444295@s.whatsapp.net', options, text, {quoted: mek})
...
//akhir kreator
		    ...
		    case prefix+'map':
			    ...
			    //if (!isPublic) return reply(mess.only.publikG)
					...
					mentions(`╔═══✪ Tag By *${pushname2}* ✪══`+ teks +'╚═══〘 BOT ARKAFSTORE 〙═══', members_id, true)
					break
			    ...
			    //if (!isPublic) return reply(mess.only.publikG)
					...
					mentions(`╔══〘  *${body.slice(12)}*  〙✪══`+teks+'╚═〘 BOT ARKAFSTORE 〙', members_id, true)
					break
			    case prefix+'kbbi':
			    if (isBanned) return reply(mess.only.benned)    
			    if (!isUser) return reply(mess.only.userB)
				...
				case prefix+'clearall':
					...
					reply(`*_Sukses delete all chat BOT ARKAFSTORE *`)
					break
                                case prefix+'bcgc':
					dms.updatePresence(from, Presence.composing) 
					if (!isOwner) return reply(mess.only.ownerB)
				...
				case prefix+'bc':
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							sendMess(_.jid, `[ *BOT ARKAFSTORE BROADCAST* ]\n\n${body.slice(4)}`)
						}
						reply('Suksess broadcast')
					}
					break
